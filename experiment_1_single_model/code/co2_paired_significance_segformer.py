...
CO2 = f"{R}/results/lora_paper/segformer_co2"
...
def main():
        ...
        if HAVE_SCIPY:
            ...
            try:
                ...
                print(f"    Wilcoxon      : W = {w:.1f},  p = {p_w:.4f}  "
                      f"(n={len(SEEDS)} -> min possible two-sided p = 0.0625)")
            except Exception as e:
    ...
    print("\n" + "=" * 78)
    print(f"NOTE: n={len(SEEDS)} seeds; Wilcoxon two-sided p floors at 0.0625, so it "
          "cannot reach 0.05 -> lean on the paired t-test + bootstrap CI + effect size.")
