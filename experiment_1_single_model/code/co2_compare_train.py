# -*- coding: utf-8 -*-
"""
Measure the CO2 / energy of ONE training run (FRE full-FT or ST-LoRA) with CodeCarbon, WITHOUT
modifying the training scripts. We import the target trainer module and call its main() inside an
OfflineEmissionsTracker (country=DEU, no network). Both trainers here use the SAME aug+CutMix
pipeline, batch size, cadence, and 1 GPU — the only difference is full-FT vs LoRA — so the
resulting energies are directly comparable.

Runs in the ISOLATED ssl_cc env (ssl clone + codecarbon). SLURM only.
Emits <out_dir>/emissions_<method>_seed<seed>.csv (codecarbon) and prints a JSON energy summary.
"""
import os
import sys
import json
...
def build_argv(method, a):
    ...
    if method == "fre":
        return common
    # ST-LoRA — deployed final_model config, aug trainer
    return common + ["--lora_r", "32", "--lora_alpha", "32", "--lora_dropout", "0.1",
                     "--no_dora", "--target_modules", "k_proj", "v", "k", "q_proj",
                     "dense", "class_predictor"]


def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("--method", choices=["fre", "stlora"], required=True)
    ...
    tracker = OfflineEmissionsTracker(
        ...
        project_name=f"pepper_{a.method}",
        log_level=a.log_level,
        save_to_file=True,
    )

    if a.method == "fre":
        import mask2former_full_train_seeded as mod
    else:
        import mask2former_lora_train_seeded_aug as mod
