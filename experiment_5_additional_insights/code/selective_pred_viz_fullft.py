# -*- coding: utf-8 -*-
"""
Selective Prediction — BG/FG Visualisation — Full FT Snapshot Ensemble
=======================================================================
Mirrors selective_pred_viz.py exactly but loads full model state dicts.

Usage:
    python selective_pred_viz_fullft.py --seed 42
"""
...
def load_snapshot(pretrained_name, checkpoint_path, device):
...
@torch.no_grad()
def collect_bg_fg_pixels(checkpoint_paths, pretrained_name, dataloader, device, H, W):
    M = len(checkpoint_paths)
    bg_conf_acc = bg_risk_acc = fg_conf_acc = fg_risk_acc = None

    for m, ckpt in enumerate(checkpoint_paths):
        model = load_snapshot(pretrained_name, ckpt, device)
        bg_conf_m, bg_risk_m, fg_conf_m, fg_risk_m = [], [], [], []
        ...
        bg_c = torch.cat(bg_conf_m).numpy().astype(np.float64)
        bg_r = torch.cat(bg_risk_m).numpy().astype(np.float64)
        fg_c = torch.cat(fg_conf_m).numpy().astype(np.float64)
        fg_r = torch.cat(fg_risk_m).numpy().astype(np.float64)
...
# ─────────────────────────────────────────────────────────────────────────────
# Curves and metrics (identical to LoRA viz)
# ─────────────────────────────────────────────────────────────────────────────
...
def compute_risk_coverage_curve(conf, risk, n_points=200):
    ...
    return coverages, risks_at_cov, float(risk.mean())


def compute_aurc(conf, risk):
    n = len(conf)
    order = np.argsort(conf)
    return float((np.cumsum(risk[order]) / np.arange(1, n + 1)).mean())


# ─────────────────────────────────────────────────────────────────────────────
# Plots (identical structure to LoRA viz)
# ─────────────────────────────────────────────────────────────────────────────
...
def plot_entropy_histograms(bg, fg, out_path, sample_size=1_000_000):
    ...
    ax.set_title("Entropy Distribution — Full FT", fontsize=LABEL_FONT)
    ...
    def bootstrap_std(arr, n_boot=200):
        return np.std([rng2.choice(arr, size=len(arr), replace=True).mean() for _ in range(n_boot)])

    means  = [bg_risk.mean(), fg_risk.mean()]
    stds   = [bootstrap_std(bg_risk), bootstrap_std(fg_risk)]
    ...
    ax.set_title("Risk by Region — Full FT", fontsize=LABEL_FONT)
    ...
    fig.suptitle("Full FT — Background Has High Entropy but Low Risk", fontsize=LABEL_FONT, y=1.02)
    fig.tight_layout()
...
def plot_risk_coverage_curves(bg, fg, out_path):
    ...
    for ax, cov, rc, base, aurc, color, label in [
        (axes[0], bg_cov, bg_rc, bg_base, bg_aurc, "#1f77b4", "Background — Full FT"),
        (axes[1], fg_cov, fg_rc, fg_base, fg_aurc, "#ff7f0e", "Foreground — Full FT"),
    ]:
        ...
        ax.set_title(f"{label}", fontsize=LABEL_FONT)
...
def plot_combined_curves(bg, fg, out_path):
    ...
    ax.plot(bg_cov, bg_rc, color="#1f77b4", linewidth=2.0,
            label=f"Background (AURC={compute_aurc(bg['conf'], bg['risk']):.4f}, base={bg_base:.4f})")
    ax.plot(fg_cov, fg_rc, color="#ff7f0e", linewidth=2.0,
            label=f"Foreground (AURC={compute_aurc(fg['conf'], fg['risk']):.4f}, base={fg_base:.4f})")
    ...
    ax.set_xlabel("Coverage", fontsize=LABEL_FONT)
    ax.set_ylabel("Risk (pixel error rate)", fontsize=LABEL_FONT)
    ax.set_title("Full FT — Risk/Coverage: Background vs Foreground", fontsize=LABEL_FONT)
    ax.set_xlim(0, 1); ax.set_ylim(bottom=0)
    ax.tick_params(labelsize=TICK_FONT, length=TICK_SIZE)
    ax.grid(True, linestyle=GRID_STYLE, alpha=GRID_ALPHA)
    ax.spines["top"].set_visible(False); ax.spines["right"].set_visible(False)
...
def main():
    ...
    checkpoint_paths = [os.path.join(seed_dir, f"model_shot_{s}.pt") for s in args.shot_ids]
    ...
    print(f"\n[Full FT] Collecting BG/FG pixels  (seed={args.seed})...")
    bg, fg = collect_bg_fg_pixels(checkpoint_paths, args.pretrained,
                                   dataloader, device, args.height, args.width)
    ...
    prefix = os.path.join(args.out_dir, f"fullft_sel_pred_seed{args.seed}")
