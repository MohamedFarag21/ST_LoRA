# -*- coding: utf-8 -*-
"""
Full Fine-tuning Snapshot Ensemble Evaluation — Seeded
=======================================================
Mirrors elora_evaluation_seeded.py exactly, but loads full model state dicts
(.pt files) instead of PEFT adapters.

Checkpoint format expected:
    results_dir/seed_<N>/model_shot_<K>.pt

Usage:
    python fullft_evaluation_seeded.py --seed 42
    python fullft_evaluation_seeded.py --seed 42 --results_dir /path/to/full_ft
"""
...
# ─────────────────────────────────────────────────────────────────────────────
# Model loading — full state dict, no PEFT
# ─────────────────────────────────────────────────────────────────────────────

def load_snapshot(pretrained_name, checkpoint_path, device):
    """Load base architecture and restore full fine-tuned weights from .pt file."""
...
# ─────────────────────────────────────────────────────────────────────────────
# Ensemble Inference — sequential to save GPU memory
# ─────────────────────────────────────────────────────────────────────────────

@torch.no_grad()
def run_ensemble_inference(pretrained_name, checkpoint_paths, dataloader, device, H, W):
    ...
    for m, ckpt_path in enumerate(checkpoint_paths):
        print(f"\n[{m+1}/{M}] Loading snapshot: {ckpt_path}")
...
def compute_uncertainty(ensemble_probs, weights=None):
    ...
    # Foreground-only epistemic uncertainty (MI)
    fg_stack  = ensemble_probs[:, :, 1:, :, :]
    fg_stack  = fg_stack / (fg_stack.sum(dim=2, keepdim=True) + 1e-6)
    fg_mean   = (w * fg_stack).sum(dim=0)
    H_fg_mean = -(fg_mean  * torch.log2(fg_mean  + 1e-12)).sum(dim=1)
    H_fg_per  = -(fg_stack * torch.log2(fg_stack + 1e-12)).sum(dim=2)
    w_ep      = weights.view(M, 1, 1, 1)
    epistemic = H_fg_mean - (w_ep * H_fg_per).sum(dim=0)
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Full FT Snapshot Ensemble Evaluation — Seeded")
    ...
    parser.add_argument("--shot_ids",   nargs="+", type=int, default=[1, 2, 3, 4, 5],
        help="Snapshot shot IDs (model_shot_<K>.pt)")
...
def main():
    ...
    checkpoint_paths = [
        os.path.join(seed_dir, f"model_shot_{shot}.pt")
        for shot in args.shot_ids
    ]
    missing = [p for p in checkpoint_paths if not os.path.exists(p)]
    if missing:
        raise FileNotFoundError(
            f"[Seed {args.seed}] Missing checkpoints:\n" + "\n".join(missing))

    print(f"\n{'='*60}")
    print(f"  Full FT Snapshot Evaluation")
    print(f"  Seed  : {args.seed}  |  Device: {device}")
    print(f"  Split : {args.split}  |  Snapshots: {len(checkpoint_paths)}")
    ...
    ensemble_probs, gt_maps = run_ensemble_inference(
        args.pretrained, checkpoint_paths, dataloader, device,
    ...
    print("\n" + "=" * 55)
    print(f"  Seed {args.seed} — Full FT Evaluation Results")
    print("=" * 55)
    for k in ["mIoU", "ECE", "MECE", "ACE", "MACE", "Brier", "NLL", "acc",
