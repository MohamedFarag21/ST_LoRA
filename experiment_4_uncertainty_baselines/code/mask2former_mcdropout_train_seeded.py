# -*- coding: utf-8 -*-
"""
Mask2Former + Monte Carlo Dropout Fine-tuning for Sweet Pepper Segmentation
============================================================================
...
Usage:
    python mask2former_mcdropout_train_seeded.py --seed 42
    python mask2former_mcdropout_train_seeded.py --seed 42 --gpus 4
"""
...
# ─────────────────────────────────────────────────────────────────────────────
# MC Dropout model wrapper
# ─────────────────────────────────────────────────────────────────────────────

def add_dropout_to_class_predictor(model, dropout_p: float = 0.25):
    original = model.class_predictor
    model.class_predictor = nn.Sequential(nn.Dropout(p=dropout_p), original)
    print(f"\n[MC Dropout] Class predictor modified:\n  {model.class_predictor}\n")
    return model
...
class Mask2FormerMCDropoutModule(pl.LightningModule):
    """Mask2Former fine-tuned with MC Dropout on the class predictor."""
    ...
    def __init__(
        ...
        self.model = add_dropout_to_class_predictor(base_model, dropout_p)
        total = sum(p.numel() for p in self.model.parameters())
        print(f"[MC Dropout] Total trainable parameters: {total:,}\n")
...
def parse_args():
    ...
    parser.add_argument("--log_dir",  type=str,
        default="/lustre/scratch/data/mibrahi2_hpc-my_research/mibrahi2_hpc-my_research-1783386603/mibrahi2_hpc-my_research-1775524204/results/lora_paper/mcdropout/logs")
...
def main():
    ...
    model = Mask2FormerMCDropoutModule(
    ...
    logger = TensorBoardLogger(save_dir=args.log_dir, name=f"mcdropout_seed{args.seed}")
