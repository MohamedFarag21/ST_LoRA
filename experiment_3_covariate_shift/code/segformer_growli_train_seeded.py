...
class GrowliFlowerDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.augment:
            # HETEROGENEOUS training aug (Exp 3): random rotation ~U(0,25 deg) + hflip p=0.5,
            # applied to image (bilinear) AND mask (nearest; revealed corners -> ignore 255).
            ang = float(torch.empty(1).uniform_(0.0, 25.0).item())
            pil_img = pil_img.rotate(ang, resample=Image.BILINEAR, fillcolor=(0, 0, 0))
            seg = np.array(Image.fromarray(seg).rotate(ang, resample=Image.NEAREST, fillcolor=255),
                           dtype=np.uint8)
